                    ...
                    <div className='flex flex-col mt-3'>
                        <div className={`details rounded-md ${mode === 'black' ? 'border-slate-500' : 'border-slate-300'}`}>
                            ...
                            <div className="AddStaff absolute top-[9%] right-6">
                                <Link to={"/addstaff"} className="w-[10rem] h-[2.5rem] flex justify-center items-center bg-blue-500 text-white font-bold text-[0.9rem] rounded-sm gap-2">
                                ...
                                state.map(item => {
                                    return <form key={item.id} className={`grid grid-cols-2 pt-2 px-6 gap-4 rounded-b-md ${mode === 'black' ? 'text-white' : ''}`} onSubmit={submit}>
                                        ...
                                        <div className='flex flex-col'>
                                            ...
                                            <input type='text' name='gstin' defaultValue={item.gstin === null ? '' : item.gstin} className={`border ${mode === 'black' ? 'bg-transparent border-slate-500' : 'border-slate-300'} p-2 text-[0.9rem] rounded-sm`} placeholder='21CAAXXXXXXXXX' onChange={(e) => handleEdit(e)} />
                                        </div>
                                        <div className='flex flex-col'>
                                            <label className={`text-[0.9rem] mx-1 ${mode === 'black' ? 'text-slate-300' : 'text-slate-500'}`}>FSSAI</label>
                                            <input type='text' name='fssai_number' defaultValue={item.fssai_number === null ? '' : item.fssai_number} className={`border ${mode === 'black' ? 'bg-transparent border-slate-500' : 'border-slate-300'} p-2 text-[0.9rem] rounded-sm`} placeholder='1251xxxxxxx' onChange={(e) => handleEdit(e)} />
                                        </div>
                                        <div className='flex flex-col'>
                        ...
                        hover:before:opacity-100" data-tip={`Set Your Own Prefix  { Ex : INV- , PP- , DD- }`}
                                            ...
                                            <input type='text' name='invoice_prefix' defaultValue={item.invoice_prefix === null ? '' : item.invoice_prefix} className={`border ${mode === 'black' ? 'bg-transparent border-slate-500' : 'border-slate-300'} p-2 text-[0.9rem] rounded-sm`} placeholder='1251xxxxxxx' onChange={(e) => handleEdit(e)} />
                                        </div>
                                    </form>
                                })}
                        </div>
                        <div className="button mx-6 mt-16 flex justify-end">
